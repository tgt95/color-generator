...
window.onload = function() {
		...
		for (var i = 0; i < (100/step); i++) {
			document.querySelector('.box-result').insertAdjacentHTML('beforeend', 
				'<div class="box-result--item">'+
					...
					'<div class="item-flex">'+
						'<div class="hex"></div>'+
						'<button type="button" class="btn btn-sm btn-copy" data-toggle="tooltip" data-placement="top" title="Copied!">'+
							'<i class="typcn typcn-book"></i>'+
						'</button>'+
					'</div>'+
					'<div class="box-result--item-background"></div>'+
				'</div>'
			);
		}
	}
	...
	document.querySelectorAll('[data-toggle="tooltip"]').forEach(function(el, index){
		el.addEventListener('click', function(event){
			...
			body.insertAdjacentHTML('beforeend', 
				'<div class="tooltip tooltip-inner" style="position: absolute; top: 0; left: 0; will-change: transform; transform: translate3d('+ (pos.left + window.scrollX) +'px, '+ (pos.top + window.scrollY - 35) +'px, 0);">'+
	...
	ConvertHex.prototype.calculate = function(value){
		...
		if(value != '' || value != undefined){
			...
			hslInput.value = (fail ? '' : 'hsl(' + rgbToHsl.apply(null, rgb).join(',') + ')');

			// Add Color to Box Generator
	...
	ConvertHex.prototype.printValue = function(){
		...
		if (!result == '' && result == result) {
			var temp, originalResult, count, cutResult = [];
